...
const counterManageCommand = new SlashCommandBuilder()
   ...
   .addSubcommand(
      (sub) =>
         ...
         .addStringOption((option) => 
            option.setName('name')
               .setDescription('Make a specific counter with name. Name must be unique.')
               .setRequired(true))
         .addStringOption((option) =>
            option.setName('display')
               .setDescription('The display message when you get the value. Add \'{%d}\' to your message for the value position.')
               .setMaxLength(150)
               .setRequired(true)
         ))
   .addSubcommand(
...
const countCommand = new SlashCommandBuilder()
   ...
   .addStringOption((option) =>
      option
         .setName('name')
         .setDescription('Name of the counter to increment. Get all counter with `/counter list`')
         .setRequired(true)
   )
   ...
   interactionStream$.subscribe(async (interaction) => {
      switch(interaction.commandName) {
         ...
         case 'robo':
            ...
            const query = !seed ? encodeURI(interaction.user.username) : encodeURI(`${interaction.user.username}+${seed}`);
         ...
         case 'counter':
            switch(interaction.options.getSubcommand()) {
               ...
               case 'create':
                  ...
                  await interaction.reply(`New counter created! ${newCtr.name} -> ${newCtr.message}`);
                  break;
            }
            break;
         case 'count':
         ...
         async (wordInMessage) => {
            if (!checker.correct(wordInMessage)) {
               ...
               if (sugg.length > 0 ) {
                  ...
                  const emote = client.emojis.cache.find(emoji => emoji.name === 'Pepega');
                  await message.reply(`${emote} wat`);
               }
            }
      });
      await fuckYouToo(mess, 0.5, 
         async (replyMessage) => {
